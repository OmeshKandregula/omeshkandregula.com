...
2. **NFC tap URLs depend on `<base href="/">` in `index.html`.** `/e/<id>` and `/c/<id>` are rewritten to `/`, which leaves the document with a `/e/` base URL. The snippet deliberately delays cleaning the address bar until Vercel Analytics has counted the tap, so `<base>` is the only thing keeping relative assets resolving in that window. Remove it and every image plus `hero-scene.js` 404, but only on tapped links, so normal traffic looks fine.
